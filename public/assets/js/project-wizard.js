(() => {
    ...
    function updateTeamPolicy() {
        ...
        if (membersHelp) { const semesterLabel = config.student?.semester ? `Estudiantes disponibles de ${config.student.semester}.º semestre. ` : ''; membersHelp.hidden = individual; membersHelp.textContent = `${semesterLabel}Solo puedes agregar estudiantes activos de tu carrera, período y mismo semestre.`; }
    }
    ...
    function filterStudents() {
        const query = memberSearch?.value.trim().toLocaleLowerCase() || ''; let visible = 0, eligible = 0;
        document.querySelectorAll('[data-student-results] > label').forEach((row) => { const input = row.querySelector('input[type="checkbox"]'); const base = input && input.value !== actorId() && !input.checked && !row.hidden && (!config.student || row.dataset.semester === String(config.student.semester)); const okay = base && (!query || row.dataset.studentName.includes(query)); row.classList.toggle('is-filtered', !okay); if (base) eligible++; if (okay) visible++; });
    ...
    memberSearch?.addEventListener('input', filterStudents);
