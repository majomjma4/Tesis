...
(() => {
  ...
  const keywordLimit = form?.querySelector('[data-direct-keyword-limit]');
  const maxProjectKeywords = 8;
  ...
  const updateKeywordSelection = () => { const inputs = [...(keywordOptions?.querySelectorAll('input[type="checkbox"]') || [])]; const selected = inputs.filter(input => input.checked); const atLimit = selected.length >= maxProjectKeywords; inputs.forEach(input => { input.disabled = atLimit && !input.checked; input.closest('[role="option"]')?.setAttribute('aria-selected', String(input.checked)); }); if (keywordSummary) keywordSummary.textContent = selected.length ? `${selected.length} ${selected.length === 1 ? 'etiqueta seleccionada' : 'etiquetas seleccionadas'}` : 'Selecciona etiquetas de clasificación'; if (keywordLimit) keywordLimit.hidden = !atLimit; };
