...
async function submitPresentationChange() {
    ...
    try {
        ...
        data.set("context", fileRemoveConfig.dataset.context || "");
...
async function submitFileRemoval() {
    ...
    try {
        ...
        data.append("context", fileRemoveConfig.dataset.context || "");
...
(() => {
    ...
    const triggers = [...document.querySelectorAll("[data-project-status-transition]")];
    ...
    const open = trigger => {
        ...
        returnFocus = trigger;
        title.textContent = selected.label || "Cambiar estado académico";
        ...
        digitalRecordMenuPanel.hidden = true;
        digitalRecordMenuTrigger?.setAttribute("aria-expanded", "false");
        window.requestAnimationFrame(() => (selected.reason_required ? reason : submitButton).focus());
    };
    const submit = async () => {
        ...
        body.set("action", "change_status");
        body.set("id", overlay.dataset.projectId || "");
        body.set("expected_status", overlay.dataset.currentStatus || "");
        body.set("target_status", selected.target);
        body.set("reason", reasonValue);
        try {
    ...
    overlay.addEventListener("keydown", event => {
        ...
        if (event.key !== "Tab") return;
        const controls = [...dialog.querySelectorAll("button:not(:disabled),textarea:not(:disabled)")].filter(control => !control.hidden && control.getClientRects().length > 0);
        if (!controls.length) return;
        const index = controls.indexOf(document.activeElement);
