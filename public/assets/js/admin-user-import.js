(()=>{
    ...
    confirmBox.addEventListener('click',event=>{if(event.target===confirmBox){confirmBox.hidden=true;syncDialogs();}});
    confirmBox.querySelector('[data-accept-import-confirm]').addEventListener('click',async event=>{const confirmButton=event.currentTarget;confirmButton.disabled=true;try{const result=await send('import');close();await window.refreshAdminUsersListing?.(result.message||'Usuarios creados correctamente.');}catch(error){confirmBox.hidden=true;syncDialogs();message.className='users-message error';message.textContent=error.message;message.hidden=false;}finally{confirmButton.disabled=false;}});
