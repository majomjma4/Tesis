...
final class ThesisTribunalService
{
    ...
    public function saveInTransaction(PDO $db, int $id, string $expected, array $memberIds, string $reason, int $actor): array
    {
        ...
        $action=$current?'tribunal_updated':'tribunal_assigned'; $auditId=(new ProjectAuditService($db))->record($id,$actor,$action,'project_participants',$id,['members'=>array_values($current),'status'=>$status],['members'=>array_values($next),'status'=>$status,'reason'=>$reason?:null],$reason?:null); $added=array_values(array_diff_key($next,$current)); if($action==='tribunal_updated')(new ProjectAcademicNotificationService())->tribunalUpdated($db,$id,$p['code'],$p['title'],$added,$actor,$auditId); else (new ProjectAcademicNotificationService())->tribunalAssigned($db,$id,$p['code'],$p['title'],$added,$actor);
