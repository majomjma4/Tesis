...
final class PaginationService
{
    ...
    public static function request(string $pageKey='p',string $sizeKey='per_page',int $defaultSize=10):array
    {
        ...
        $defaultSize=self::normalizeSize($defaultSize);
        $size=self::normalizeSize((int)($_GET[$sizeKey]??$defaultSize),$defaultSize);
    ...
    public static function run(PDO $db,string $countSql,string $dataSql,array $params,array $request):array
    {
        ...
        $effectiveSize=self::normalizeSize((int)($request['size']??10));
    ...
    public static function normalizeSize(int $size,int $fallback=10):int
    {
        if(in_array($size,self::ALLOWED_SIZES,true))return $size;
        return in_array($fallback,self::ALLOWED_SIZES,true)?$fallback:10;
    }
}
