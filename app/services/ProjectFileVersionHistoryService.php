...
final class ProjectFileVersionHistoryService
{
    ...
    public function forFile(int $projectId,int $fileId,int $actorId,string $context):array
    {
        ...
        $query=$db->prepare("SELECT v.id version_id,v.version_number,v.checksum_sha256,v.storage_name,v.replaced_at,v.physical_status,v.unavailable_reason,c.id change_id,c.new_version_number,c.declared_summary,c.sections_json,c.previous_document_status,c.new_document_status,c.changed_at
            FROM project_file_versions v LEFT JOIN project_file_version_changes c ON c.previous_version_id=v.id WHERE v.project_id=:project AND v.file_id=:file AND EXISTS (SELECT 1 FROM project_deliveries d WHERE d.project_id=v.project_id AND d.submitted_at<=v.replaced_at) ORDER BY v.version_number");$query->execute(['project'=>$projectId,'file'=>$fileId]);$versions=[];
        foreach($query->fetchAll() as $row)$versions[]=$this->historical($db,$projectId,$row);
        $change=$db->prepare('SELECT * FROM project_file_version_changes c WHERE c.project_id=:project AND c.file_id=:file AND c.new_checksum=:checksum AND EXISTS (SELECT 1 FROM project_deliveries d WHERE d.project_id=c.project_id AND d.submitted_at<=c.changed_at) ORDER BY c.changed_at DESC,c.id DESC LIMIT 1');$change->execute(['project'=>$projectId,'file'=>$fileId,'checksum'=>$current['checksum_sha256']]);$currentChange=$change->fetch()?:null;
    ...
    public function accessibleVersion(int $projectId, int $versionId, int $actorId, string $context): array
    {
        ...
        $q=$db->prepare('SELECT v.*,p.deleted_at project_deleted_at,f.deleted_at file_deleted_at,f.purged_at file_purged_at FROM project_file_versions v JOIN projects p ON p.id=v.project_id JOIN project_files f ON f.id=v.file_id WHERE v.id=:version AND v.project_id=:project AND EXISTS (SELECT 1 FROM project_deliveries d WHERE d.project_id=v.project_id AND d.submitted_at<=v.replaced_at)');
