...
final class AdminController
{
    ...
    public function saveProject():void{$this->requirePost();$session=new AuthSessionService();if(!$session->validateCsrf('admin_projects',(string)($_POST['_csrf']??'')))$this->json(false,'La sesión del formulario venció.',[],419);$id=(int)($_POST['id']??0);$title=trim((string)($_POST['title']??''));$publicationIntent=(string)($_POST['publication_intent']??'')==='1';try{if((string)($_POST['action']??'')==='prepare_public_description'){$this->json(true,'Descripción pública consultada.',(new ProjectDescriptionService())->prepareForPublication($id));}$payload=['title'=>$title,'subtitle'=>trim((string)($_POST['subtitle']??'')),'summary'=>trim((string)($_POST['summary']??'')),'project_type_id'=>(int)($_POST['project_type_id']??0),'career_id'=>(int)($_POST['career_id']??0),'academic_period_id'=>(int)($_POST['academic_period_id']??0),'tutor_id'=>(int)($_POST['tutor_id']??0),'tutoring_managed'=>(string)($_POST['tutoring_managed']??'')==='1','tutoring_user_ids'=>(array)($_POST['tutoring_user_ids']??[]),'tutoring_primary_id'=>(int)($_POST['tutoring_primary_id']??0),'status'=>(string)($_POST['status']??'development'),'presentation_file_id'=>(int)($_POST['presentation_file_id']??0),'public_description'=>(string)($_POST['public_description']??''),'description_origin'=>(string)($_POST['description_origin']??''),'keywords'=>(array)($_POST['project_keywords']??[])];$saved=(new AdminProjectModel())->save($payload,$id,(int)$session->userId());$this->json(true,$publicationIntent?'El proyecto se publicó correctamente con su descripción pública.':($id?'Proyecto actualizado correctamente.':'Proyecto creado correctamente.'),['id'=>$saved]);}catch(ProjectTutoringException $exception){$this->json(false,$exception->getMessage(),[],422);}catch(InvalidArgumentException $exception){if($id)$this->activityFailure($session,'project_status_changed','Intentó modificar el estado de un proyecto','Proyectos','project',$id,$title?:'Proyecto #'.$id,$exception);$this->json(false,$exception->getMessage(),[],422);}catch(Throwable $exception){if($id)$this->activityFailure($session,'project_status_changed','Intentó modificar el estado de un proyecto','Proyectos','project',$id,$title?:'Proyecto #'.$id,$exception);error_log('Admin save project: '.$exception->getMessage());$this->json(false,$publicationIntent?'No fue posible completar la publicación. No se realizaron cambios.':'No fue posible guardar el proyecto.',[],500);}}
