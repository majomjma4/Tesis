...
<?php endif; ?>
    ...
    public function publishProject(): void
    {
        $this->requirePost();
        $s = new AuthSessionService();
        if (!$s->validateCsrf('admin_repository', (string) ($_POST['_csrf'] ?? ''))) {
        ...
        $required = match ($action) {
            'presentation', 'unpresentation' => 'manage_files',
            'availability', 'publish', 'restore', 'unpublish' => 'manage_publication',
            default => null,
        };
        if ($required === null) {
            $this->json(false, 'La acción solicitada no es válida.', [], 422);
        }
        ...
        try {
            ...
            if ($action === 'presentation' || $action === 'unpresentation') {
                $model->setPresentationFile(
                    $id,
                    $action === 'presentation' ? (int) ($_POST['file_id'] ?? 0) : null,
                    (int) $s->userId()
                );
                $this->json(
                    true,
                    $action === 'unpresentation'
            ...
            if ($action === 'availability') {
                $available = filter_var(
                    $_POST['is_available'] ?? null,
                    FILTER_VALIDATE_BOOL,
                    FILTER_NULL_ON_FAILURE
                );
                if ($available === null) {
                    $this->json(false, 'La disponibilidad solicitada no es válida.', [], 422);
                }
                $model->setAvailability($id, $available, (int) $s->userId());
                $this->json(
                    true,
                    $available ? 'Proyecto marcado como disponible.' : 'Proyecto marcado como no disponible.'
                );
            }
            if ($action === 'publish') {
                $this->json(
                    false,
                    'La publicación depende del flujo académico y no puede realizarse desde la administración.',
                    [],
                    403
                );
            }
            if ($action === 'restore') {
                $model->restorePublication($id, (int) $s->userId());
                $this->json(true, 'La publicación fue restaurada correctamente.');
            }
            if ($action === 'unpublish') {
                $model->setPublished($id, false, (int) $s->userId());
                $this->json(true, 'Proyecto retirado del repositorio. Permanece disponible en Proyectos.');
            }
            ...
            error_log('Publish project: ' . $e->getMessage());
            $this->json(false, 'No fue posible actualizar la publicación.', [], 500);
        }
    }
    ...
    public function users(): void
    {
        ...
        $temporaryPasswordForDisplay=null;
        try {
            $policyModel=new SystemSettingModel();
            if($policyModel->temporaryPasswordConfigured())$temporaryPasswordForDisplay=$policyModel->temporaryPasswordPolicy()['password'];
        }catch(Throwable){
            // Una policy pendiente o inválida no debe impedir cargar el panel.
        }
        $session=new AuthSessionService();
        View::render('admin/users',['currentPage'=>'admin-users','title'=>'Usuarios | Administración','bodyClass'=>'admin-users-page','pageStyles'=>[asset('css/admin-users.css'),asset('css/admin-user-import.css')],'pageScript'=>asset('js/admin-users.js'),'pageScripts'=>[asset('js/admin-user-import.js')],'users'=>$users,'pagePagination'=>$pagination,'userSummary'=>$summary,'catalogs'=>$catalogs,'filters'=>$filters,'adminUserCsrf'=>$session->csrfToken('admin_users'),'adminUserEndpoints'=>['save'=>route('admin-user-save'),'status'=>route('admin-user-status'),'password'=>route('admin-user-password'),'import'=>route('admin-users-import')],'adminUsersError'=>$error,'temporaryPasswordForDisplay'=>$temporaryPasswordForDisplay]);
    }
