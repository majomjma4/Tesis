...
final class AdminTrashModel
{
 ...
 public function restoreSupportMaterial(int $id,int $actor):void
 {
  ...
  public function supportMaterialDashboard(array $pagination=[]):array{
   ...
   $data="SELECT sm.id,sm.title,sm.material_type code,sm.deleted_at,sm.deletion_reason,u.full_name deleted_by_name,DATE_ADD(sm.deleted_at,INTERVAL ".$mDays." DAY) expires_at,GREATEST(0,CEIL(TIMESTAMPDIFF(SECOND,UTC_TIMESTAMP(),DATE_ADD(sm.deleted_at,INTERVAL ".$mDays." DAY))/86400)) days_left".$from.' ORDER BY sm.deleted_at DESC';
   ...
   return ['loaded'=>true,'candidates'=>[],'users'=>[],'projects'=>[],'materials'=>$result['items'],'pagination'=>$result['pagination'],'active_type'=>'materials','retention'=>['users'=>(int)($settings['retention_users_days']??60),'projects'=>(int)($settings['retention_projects_days']??60),'materials'=>$mDays],'summary'=>['users'=>$uCount,'projects'=>$pCount,'materials'=>$mCount,'expired'=>(int)$expiredStmt->fetchColumn(),'total'=>$uCount+$pCount+$mCount]];
  }
  ...
  public function dashboard(string $type='users',array $pagination=[]):array{
   ...
   if($type==='projects'){
    ...
    $data="SELECT p.id,p.code,p.title,p.deleted_at,p.deletion_reason,u.full_name deleted_by_name,DATE_ADD(p.deleted_at,INTERVAL ".$pDays." DAY) expires_at,GREATEST(0,CEIL(TIMESTAMPDIFF(SECOND,UTC_TIMESTAMP(),DATE_ADD(p.deleted_at,INTERVAL ".$pDays." DAY))/86400)) days_left".$from.' ORDER BY p.deleted_at DESC';
   }else{
    $from=" FROM users u LEFT JOIN users a ON a.id=u.deleted_by WHERE u.deleted_at IS NOT NULL AND u.purged_at IS NULL";
    $data="SELECT u.id,u.full_name,u.email,u.deleted_at,u.deletion_reason,a.full_name deleted_by_name,DATE_ADD(u.deleted_at,INTERVAL ".$uDays." DAY) expires_at,GREATEST(0,CEIL(TIMESTAMPDIFF(SECOND,UTC_TIMESTAMP(),DATE_ADD(u.deleted_at,INTERVAL ".$uDays." DAY))/86400)) days_left".$from.' ORDER BY u.deleted_at DESC';
   }
   $result=PaginationService::run($d,'SELECT COUNT(*)'.$from,$data,[],$pagination?:PaginationService::request());
   return ['loaded'=>true,'candidates'=>$candidates,'users'=>$type==='users'?$result['items']:[],'projects'=>$type==='projects'?$result['items']:[],'materials'=>[],'pagination'=>$result['pagination'],'active_type'=>$type,'retention'=>['users'=>$uDays,'projects'=>$pDays,'materials'=>$mDays],'summary'=>['users'=>$userCount,'projects'=>$projectCount,'materials'=>$materialCount,'expired'=>$expired,'total'=>$userCount+$projectCount+$materialCount]];
  }
  public function trashUser(int $id,string $reason,int $actor):void{if($id<1||mb_strlen(trim($reason))<5)throw new InvalidArgumentException('Indica un motivo de al menos cinco caracteres.');Database::transaction(function(PDO $d)use($id,$reason,$actor):void{$read=$d->prepare('SELECT id,status,is_admin,is_initial_admin FROM users WHERE id=:id AND deleted_at IS NULL AND purged_at IS NULL FOR UPDATE');$read->execute(['id'=>$id]);$user=$read->fetch();if(!$user)throw new InvalidArgumentException('El usuario ya no está disponible.');if((bool)$user['is_admin']&&$user['status']==='active'){$admins=$d->prepare("SELECT COUNT(*) FROM users WHERE is_admin=1 AND status='active' AND deleted_at IS NULL AND purged_at IS NULL AND id<>:id");$admins->execute(['id'=>$id]);if((int)$admins->fetchColumn()<1)throw new InvalidArgumentException(AdminUserModel::LAST_ADMIN_MESSAGE);}$q=$d->prepare('UPDATE users SET status=\'inactive\',deleted_at=CURRENT_TIMESTAMP,deleted_by=:actor,deletion_reason=:reason,session_version=session_version+1 WHERE id=:id');$q->execute(['actor'=>$actor,'reason'=>trim($reason),'id'=>$id]);$this->audit($d,$actor,'user_trashed','user',$id,['reason'=>trim($reason),'previous_status'=>(string)$user['status']]);if((bool)$user['is_initial_admin'])$this->audit($d,$actor,'initial_admin_deactivated','user',$id,['reason'=>trim($reason),'via'=>'trash']);});}
  public function restore(string $entity,int $id,int $actor):void{if(!in_array($entity,['user','project'],true)||$id<1)throw new InvalidArgumentException('El elemento no es válido.');Database::transaction(function(PDO $d)use($entity,$id,$actor):void{$restoredAdmin=false;$restoreStatus='active';if($entity==='user'){$read=$d->prepare('SELECT is_admin FROM users WHERE id=:id AND deleted_at IS NOT NULL AND purged_at IS NULL FOR UPDATE');$read->execute(['id'=>$id]);$restoredAdmin=(bool)$read->fetchColumn();$restoreStatus=$this->previousUserStatus($d,$id);$q=$d->prepare("UPDATE users SET status=:status,deleted_at=NULL,deleted_by=NULL,deletion_reason=NULL,session_version=session_version+1 WHERE id=:id AND deleted_at IS NOT NULL AND purged_at IS NULL");$q->bindValue(':status',$restoreStatus);$q->bindValue(':id',$id,PDO::PARAM_INT);}else{$q=$d->prepare('UPDATE projects SET deleted_at=NULL,deleted_by=NULL,deletion_reason=NULL WHERE id=:id AND deleted_at IS NOT NULL');$q->bindValue(':id',$id,PDO::PARAM_INT);}$q->execute();if($q->rowCount()!==1)throw new InvalidArgumentException('El elemento ya no puede restaurarse.');$this->audit($d,$actor,$entity.'_restored',$entity,$id,['restored_status'=>$restoreStatus]);if($restoredAdmin&&$restoreStatus==='active')$this->audit($d,$actor,'admin_access_reactivated','user',$id,['via'=>'trash_restore']);});}
  ...
  public function purgeExpiredAutomatically(bool $dryRun=false):array
  {
   ...
   $result=['dry_run'=>$dryRun,'processed'=>0,'deleted'=>0,'failed'=>0,'skipped'=>0,'users'=>0,'projects'=>0,'materials'=>0,'items'=>[],'failures'=>[],'filesystem_cleanup_pending'=>[]];
   if(!$dryRun)$this->cleanupResiduals($result);
   foreach($days as $entity=>$retention){
    foreach($this->expiredEntries($entity,$retention) as $item){
     ...
     try{$this->batch($entity,[(int)$item['id']],0,true,true);$result['deleted']++;$result[$entity]++;$result['filesystem_cleanup_pending']=array_merge($result['filesystem_cleanup_pending'],$this->consumePhysicalCleanupPending());}
     catch(Throwable $e){$result['failed']++;$result['failures'][]=['entity'=>$entity,'id'=>(int)$item['id'],'message'=>'No fue posible procesar el elemento.'];$result['filesystem_cleanup_pending']=array_merge($result['filesystem_cleanup_pending'],$this->consumePhysicalCleanupPending());error_log('Automatic trash purge '.$entity.' #'.$item['id'].': '.$e->getMessage());(new AdminActivityService())->recordFailure(0,'trash_auto_purge_failed','Falló la eliminación automática de Papelera','Papelera',$entity,(int)$item['id'],(string)$item['label'],$e);}
    }
   }
  ...
  private function notifyAutomaticPurge(array $result):void
  {
   ...
   $message='Se eliminaron definitivamente '.$total.' elemento'.($total===1?'':'s').' que cumplieron el periodo de retención configurado: '.implode(', ',$summary).'.';if(!empty($result['failed']))$message.=' El proceso finalizó con observaciones.';
  ...
  private function batch(string $entity,array $ids,int $actor,bool $permanent,bool $automatic=false):int
  {
   ...
   catch(Throwable $e){foreach($stages as $id=>$stage)try{$storage->restore($stage);}catch(Throwable $restoreError){$this->physicalCleanupPending[]=['entity'=>$entity,'id'=>(int)$id];error_log('Trash staging compensation '.$entity.' #'.$id.': '.$restoreError->getMessage());}throw $e;}
   foreach($stages as $id=>$stage)try{$storage->destroy($stage);}catch(Throwable $e){$this->physicalCleanupPending[]=['entity'=>$entity,'id'=>(int)$id];error_log('Trash physical purge anomaly '.$entity.' #'.$id.': '.$e->getMessage());}return $count;
   }
  ...
  private function assertAllTrashed(PDO $d,string $entity,array $ids):void{$table=$entity==='users'?'users':($entity==='projects'?'projects':'support_materials');$extra=$entity==='projects'?'':' AND purged_at IS NULL';$ph=implode(',',array_fill(0,count($ids),'?'));$q=$d->prepare('SELECT id FROM '.$table.' WHERE id IN ('.$ph.') AND deleted_at IS NOT NULL'.$extra.' FOR UPDATE');$q->execute($ids);if(count($q->fetchAll(PDO::FETCH_COLUMN))!==count($ids))throw new InvalidArgumentException('Uno o más elementos ya no están disponibles en la Papelera.');}
   private function restoreAtomic(PDO $d,string $entity,int $id,int $actor):void{if($entity==='users'){$status=$this->previousUserStatus($d,$id);$q=$d->prepare("UPDATE users SET status=:status,deleted_at=NULL,deleted_by=NULL,deletion_reason=NULL,session_version=session_version+1 WHERE id=:id AND deleted_at IS NOT NULL AND purged_at IS NULL");$q->bindValue(':status',$status);$q->bindValue(':id',$id,PDO::PARAM_INT);}else{$valid=$d->prepare('SELECT 1 FROM users WHERE id=(SELECT created_by FROM projects WHERE id=:id) AND deleted_at IS NULL AND purged_at IS NULL');$valid->execute(['id'=>$id]);if(!$valid->fetchColumn())throw new InvalidArgumentException('No puede restaurarse el proyecto porque su autor ya no está disponible.');$q=$d->prepare('UPDATE projects SET deleted_at=NULL,deleted_by=NULL,deletion_reason=NULL WHERE id=:id AND deleted_at IS NOT NULL');$q->bindValue(':id',$id,PDO::PARAM_INT);$status='active';}$q->execute();if($q->rowCount()!==1)throw new RuntimeException('El elemento cambió de estado durante la restauración.');$this->audit($d,$actor,$entity==='users'?'user_restored':'project_restored',$entity==='users'?'user':'project',$id,['restored_status'=>$status]);}
   private function previousUserStatus(PDO $d,int $id):string{$q=$d->prepare("SELECT details FROM admin_audit_log WHERE action='user_trashed' AND entity_type='user' AND entity_id=:id AND result='correct' ORDER BY created_at DESC,id DESC LIMIT 1");$q->execute(['id'=>$id]);$details=$q->fetchColumn();if(is_string($details)&&$details!==''){$decoded=json_decode($details,true);$status=is_array($decoded)?(string)($decoded['previous_status']??''):'';if(in_array($status,['active','inactive','blocked'],true))return $status;}return 'inactive';}
  ...
  public function purgeExpired(int $actor):array{
   $settings=(new SystemSettingModel())->all();$days=['users'=>max(1,(int)($settings['retention_users_days']??60)),'projects'=>max(1,(int)($settings['retention_projects_days']??60)),'materials'=>max(1,(int)($settings['retention_materials_days']??60))];$result=['users'=>0,'projects'=>0,'materials'=>0,'failed'=>0,'failures'=>[],'filesystem_cleanup_pending'=>[]];
   $this->cleanupResiduals($result);
   foreach($days as $entity=>$retention){foreach($this->expiredEntries($entity,$retention) as $item){try{$this->batch($entity,[(int)$item['id']],$actor,true);$result[$entity]++;$result['filesystem_cleanup_pending']=array_merge($result['filesystem_cleanup_pending'],$this->consumePhysicalCleanupPending());}catch(Throwable $e){$result['failed']++;$result['failures'][]=['entity'=>$entity,'id'=>(int)$item['id']];$result['filesystem_cleanup_pending']=array_merge($result['filesystem_cleanup_pending'],$this->consumePhysicalCleanupPending());error_log('Manual trash purge '.$entity.' #'.$item['id'].': '.$e->getMessage());}}}
   return $result;
  }
 private function cleanupResiduals(array &$result):void{$storage=new TrashStoragePurgeService();foreach(['project'=>'storage/private/projects','material'=>'storage/support-materials'] as $kind=>$relative){$holding=ROOT_PATH.'/'.$relative.'/.trash-purge';if(!is_dir($holding))continue;try{$entries=new DirectoryIterator($holding);foreach($entries as $entry){if($entry->isDot()||!$entry->isDir())continue;$stage=['staged'=>$entry->getPathname()];try{$storage->destroy($stage);}catch(Throwable $e){$result['filesystem_cleanup_pending'][]=['entity'=>$kind,'name'=>$entry->getFilename()];error_log('Trash residual cleanup '.$kind.': '.$e->getMessage());}}}catch(Throwable $e){$result['filesystem_cleanup_pending'][]=['entity'=>$kind,'name'=>'directory'];error_log('Trash residual scan '.$kind.': '.$e->getMessage());}}}
