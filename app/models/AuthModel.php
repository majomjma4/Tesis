...
final class AuthModel
{
    ...
    {
        $q=Database::connection()->prepare('SELECT u.id,u.full_name,u.email,u.created_at,u.last_login_at,u.password_changed_at,u.is_admin,u.is_initial_admin,sp.institutional_code student_institutional_code,tp.institutional_code teacher_institutional_code FROM users u LEFT JOIN student_profiles sp ON sp.user_id=u.id LEFT JOIN teacher_profiles tp ON tp.user_id=u.id WHERE u.id=:id AND u.deleted_at IS NULL AND u.purged_at IS NULL');$q->execute(['id'=>$userId]);$profile=$q->fetch();if(!$profile)throw new RuntimeException('La cuenta no existe.');$profile['roles']=$this->effectiveRoles((int)$profile['id'],(bool)$profile['is_admin']);$profile['institutional_code']=in_array('student',$profile['roles'],true)?($profile['student_institutional_code']??null):(in_array('teacher',$profile['roles'],true)?($profile['teacher_institutional_code']??null):null);return $profile;
    }
