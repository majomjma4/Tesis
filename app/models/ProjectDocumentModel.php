...
final class ProjectDocumentModel
{
    ...
    public function restorable(int $projectId):array
    {
        ...
        $q=$this->db->prepare("SELECT f.*,u.full_name deleted_by_name,TIMESTAMPDIFF(SECOND,UTC_TIMESTAMP(),DATE_ADD(f.deleted_at,INTERVAL ".$hours." HOUR)) remaining_seconds FROM project_files f LEFT JOIN users u ON u.id=f.deleted_by WHERE f.project_id=:id AND f.deleted_at IS NOT NULL AND f.purged_at IS NULL AND f.deleted_at>=DATE_SUB(UTC_TIMESTAMP(),INTERVAL ".$hours." HOUR) ORDER BY f.deleted_at DESC");$q->execute(['id'=>$projectId]);
    ...
    public function inspectRestore(int $projectId,int $fileId):array
    {
        $settings=(new SystemSettingModel())->all();
        $hours=max(1,(int)($settings['withdrawn_file_restore_hours']??24));
        $q=$this->db->prepare("SELECT * FROM project_files WHERE project_id=:project AND id=:file AND deleted_at IS NOT NULL AND purged_at IS NULL AND deleted_at>=DATE_SUB(UTC_TIMESTAMP(),INTERVAL ".$hours." HOUR) FOR UPDATE");$q->execute(['project'=>$projectId,'file'=>$fileId]);$f=$q->fetch();if(!$f)throw new InvalidArgumentException('El plazo de recuperación de este archivo finalizó.');
