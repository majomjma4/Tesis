...
final class ProjectRecordModel
{
    public function academicHistoryPage(int $projectId, int $offset = 0, int $limit = 15): array
    {
        ...
        $activitySql = "SELECT pal.id,pal.action,pal.previous_state,pal.new_state,pal.effective_context,pal.created_at,u.full_name actor_name FROM project_audit_log pal LEFT JOIN users u ON u.id=pal.user_id WHERE pal.project_id=:id AND pal.action IN ('project_updated','project_approved','project_tribunal_approved','tribunal_approved','project_published','project_unpublished','project_republished','project_publication_reverted','project_corrections_requested') ORDER BY pal.created_at,pal.id";
    ...
    public function find(int $projectId, ?int $userId, bool $administrator, bool $publishedOnly = false, bool $institutionalTeacherReadOnly = false, bool $studentOwnershipOnly = false, bool $teacherOwnedDirectReadOnly = false): ?array
    {
        ...
        $project['participants'] = $this->rows($db, "SELECT pp.user_id,pp.role_code,pp.tribunal_position,pp.permission_level,pp.is_leader,pp.assigned_at,
        ...
        $project['activity'] = $this->rows($db, "SELECT pal.id,pal.action,pal.entity_type,pal.entity_id,pal.previous_state,pal.new_state,pal.reason,pal.effective_context,pal.created_at,u.full_name AS actor_name FROM project_audit_log pal LEFT JOIN users u ON u.id=pal.user_id WHERE pal.project_id=:id ORDER BY pal.created_at ASC,pal.id ASC", $projectId);
