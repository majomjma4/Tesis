...
final class SystemSettingModel
{
    ...
    public function save(array $input,int $actor):array
    {
        ...
        $retentions=['retention_users_days'=>(int)($input['retention_users_days']??0),'retention_projects_days'=>(int)($input['retention_projects_days']??0),'retention_materials_days'=>(int)($input['retention_materials_days']??0)];
        ...
        foreach($retentions as $days)if($days<1||$days>365)throw new InvalidArgumentException('La retención debe estar entre 1 y 365 días.');
        ...
        $changed=Database::transaction(function(PDO $d)use($submitted,$actor):array{
            ...
            foreach($changed as $key=>$values){
                ...
                $activity->record($actor,'settings_updated',$secret?'Política de contraseña temporal actualizada':'Actualizó '.$this->label($key),'Configuración','settings',null,$secret?'Política de contraseña temporal':$this->label($key),'correct',$secret?['setting_key'=>'temporary_password','sensitive'=>true]:['setting_key'=>$key,'previous_value'=>$values['old'],'new_value'=>$values['new']]);
            }
            return $changed;
        });
    ...
    private function label(string $key):string{return ['institution_name'=>'Nombre de la institución','file_max_mb'=>'Límite por archivo','file_total_max_mb'=>'Límite total por entrega','file_extensions'=>'Formatos permitidos (Borrador)','file_extensions_private'=>'Formatos permitidos en Borrador','file_extensions_project'=>'Formatos permitidos en Proyecto','file_extensions_support'=>'Formatos permitidos en Materiales de apoyo','project_code_prefixes'=>'Prefijos de proyectos','project_code_digits'=>'Dígitos de códigos de proyectos','temporary_password_days'=>'Vigencia de contraseña temporal','temporary_password_force_change'=>'Cambio obligatorio de contraseña temporal','retention_users_days'=>'Retención de usuarios','retention_projects_days'=>'Retención de proyectos','retention_materials_days'=>'Retención de materiales'][$key]??$key;}
