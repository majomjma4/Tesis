...
<main class="digital-record" data-digital-record data-record-id="<?= (int) ($digitalRecord['entity']['id'] ?? 0) ?>" data-entity-type="<?= e((string) ($digitalRecord['entity']['type'] ?? 'record')) ?>" data-active-tab="<?= e($activeTab) ?>" data-persistent-tabs="<?= ($digitalRecord['mode'] ?? 'view') === 'view' ? 'true' : 'false' ?>">
    ...
    <article class="ed-shell" aria-labelledby="digitalRecordTitle">
        ...
        <nav class="ed-tabs" aria-label="Secciones del expediente"<?= ($digitalRecord['mode'] ?? 'view') === 'view' ? ' role="tablist"' : '' ?>>
            <?php foreach ($tabs as $tabItem): ?><a class="ed-tab" data-record-tab-link data-tab-id="<?= e($tabItem['id']) ?>" href="<?= e($tabItem['url']) ?>"<?= $tabItem['id'] === $activeTab ? ' aria-current="page"' : '' ?><?= ($digitalRecord['mode'] ?? 'view') === 'view' ? ' role="tab" aria-selected="' . ($tabItem['id'] === $activeTab ? 'true' : 'false') . '" aria-controls="recordTabPanel-' . e($tabItem['id']) . '"' : '' ?>><i class="fa-solid <?= e($tabItem['icon']) ?>" aria-hidden="true"></i><?= e($tabItem['label']) ?></a><?php endforeach; ?>
        </nav>
        <?php if (($digitalRecord['mode'] ?? 'view') === 'view'): ?>
            <?php foreach (['information', 'files', 'evolution'] as $tabPanel): ?>
                <section class="ed-content ed-tab-panel" id="recordTabPanel-<?= e($tabPanel) ?>" data-record-tab-panel="<?= e($tabPanel) ?>" role="tabpanel"<?= $tabPanel !== $activeTab ? ' hidden' : '' ?>>
                    <?php
