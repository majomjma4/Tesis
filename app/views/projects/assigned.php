                    ...
                    <div class="ar-grid assigned-projects-grid" data-assigned-grid="<?= e($tab) ?>">
                        <?php foreach ($assignedProjects as $project): if ($project['tab'] !== $tab) continue; ?>
                            ...
                            <article class="ar-project-card assigned-project-card" data-assigned-card data-tab="<?= e($tab) ?>" data-type-code="<?= e((string)$project['type_code']) ?>" data-period-id="<?= e((string)$project['period_id']) ?>" data-relations="<?= e(implode(' ', $relationCodes)) ?>" data-search="<?= e(mb_strtolower(implode(' ', [$project['code'],$project['title'],$project['students_search'],$project['status'],$project['period'],(string)($project['publisher'] ?? ''),implode(' ', $relationLabels)]), 'UTF-8')) ?>">
                                ...
                                <?php if (!empty($project['is_direct_repository'])): ?><p class="assigned-direct-origin"><i class="fa-solid fa-book-open" aria-hidden="true"></i> Publicado directamente en el repositorio<?php if (!empty($project['publisher'])): ?> · Publicado por: <?= e((string) $project['publisher']) ?><?php endif; ?></p><?php endif; ?>
                                ...
                                <?php $package = (array)($project['package'] ?? []); $isPublished = (string)($project['status_key'] ?? '') === 'published'; $projectReturnUrl = route('assigned-projects') . '&tab=' . $tab; $projectUrl = $isPublished ? route('repository-detail') . '&id=' . (int)$project['id'] . '&return=' . rawurlencode($projectReturnUrl) : route('project-detail') . '&id=' . (int)$project['id'] . '&return=' . rawurlencode($projectReturnUrl); ?><footer class="assigned-project-actions"><a class="ar-primary-action" href="<?= e($projectUrl) ?>"><i class="fa-regular fa-eye" aria-hidden="true"></i> <?= $isPublished ? 'Abrir ficha institucional' : 'Ver proyecto' ?></a><?php if (!empty($package['available']) && !empty($package['download_url'])): ?><a class="ar-icon-action" href="<?= e((string)$package['download_url']) ?>" data-tooltip="Descargar ZIP<?= !empty($package['size']) ? ' · ' . e((string) $package['size']) : '' ?>" aria-label="Descargar paquete ZIP del proyecto"><i class="fa-solid fa-download" aria-hidden="true"></i></a><?php endif; ?></footer>
                            </article>
                        <?php endforeach; ?>
                    </div>
