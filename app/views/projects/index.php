        ...
        <div class="projects-section" aria-labelledby="projects-<?= e(strtolower(str_replace(' ', '-', $group['title']))) ?>">
            ...
            ?>
                ...
                <article class="projects-card" data-project-card data-search="<?= e($search) ?>" data-status="<?= e($project['status_key']) ?>" data-situation="<?=$hasPendingReview?'pending':'none'?>" data-type="<?= e($project['type_key']) ?>" data-period="<?= e($project['period']) ?>" data-metric="<?= e($project['metric_bucket']) ?>" data-title="<?= e($project['title']) ?>" data-progress="<?= (int) ($project['progress'] ?? 0) ?>" data-activity="<?= (int) ($project['activity_order'] ?? 0) ?>">
                    ...
                    <?php $canPublish = (($project['type_key'] ?? '') === 'thesis' && ($project['status_key'] ?? '') === 'tribunal_approved') || (($project['type_key'] ?? '') !== 'thesis' && ($project['status_key'] ?? '') === 'approved'); ?>
                    <footer><a href="<?= e($actionUrl) ?>"><?= e($cardAction) ?> <i class="fa-solid fa-arrow-right"></i></a><?php if ($canPublish): ?><button type="button" class="projects-publish-action" data-project-publish data-project-id="<?= (int) $project['id'] ?>"><i class="fa-solid fa-upload" aria-hidden="true"></i>Publicar</button><?php endif; ?></footer>
                </article>
            <?php endforeach; ?>
            </div><?php endif; ?>
        </div></section>
