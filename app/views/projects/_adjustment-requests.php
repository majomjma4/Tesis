...
<aside class="project-adjustment-banner" data-adjustment-banner role="status" aria-labelledby="projectAdjustmentBannerTitle">
    ...
    <div><strong id="projectAdjustmentBannerTitle">Solicitud administrativa pendiente</strong><p><?=(int)$adjustmentSummary['pending_count']?> solicitud<?=((int)$adjustmentSummary['pending_count']===1?'':'es')?> pendiente<?=((int)$adjustmentSummary['pending_count']===1?'':'s')?>.</p>
...
<?php endif; ?>
<?php if($pendingAdjustments || (!empty($projectCapabilities['close_adjustment_request']) && $addressedAdjustments)): ?>
<section class="project-adjustment-list" aria-labelledby="projectAdjustmentListTitle"><header><h2 id="projectAdjustmentListTitle">Solicitudes administrativas</h2><p>Gestión separada de las observaciones académicas del Tutor.</p></header>
<?php foreach(array_merge($pendingAdjustments, !empty($projectCapabilities['close_adjustment_request']) ? $addressedAdjustments : []) as $item): ?>
<article data-adjustment-request="<?=(int)$item['id']?>"><div><span class="project-adjustment-status is-<?=e((string)$item['status'])?>"><?=($item['status']==='addressed'?'Atendida':'Pendiente')?></span><h3><?=e($typeLabels[$item['request_type']]??'Ajuste administrativo')?></h3><p><?=e((string)$item['message'])?></p><small><?=e((string)$item['requested_by_name'])?> · <?=e($adjustmentDate($item['created_at']??null))?></small><?php if(!empty($item['related_section'])):?><small>Sección: <?=e((string)$item['related_section'])?></small><?php endif;?><?php foreach((array)($item['responses']??[]) as $response):?><blockquote><strong><?=e((string)$response['author_name'])?> respondió:</strong> <?=e((string)$response['message'])?></blockquote><?php endforeach;?></div>
...
<div class="project-adjustment-dialog" id="projectAdjustmentDialog" data-adjustment-dialog hidden>
 <section role="dialog" aria-modal="true" aria-labelledby="projectAdjustmentTitle" aria-describedby="projectAdjustmentHelp">
  <header><div><span>Gestión administrativa</span><h2 id="projectAdjustmentTitle">Solicitar ajuste</h2></div><button type="button" data-adjustment-cancel aria-label="Cerrar diálogo"><i class="fa-solid fa-xmark" aria-hidden="true"></i></button></header>
