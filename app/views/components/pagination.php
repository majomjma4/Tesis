<?php if(($pagination['total']??0)>10):$page=(int)$pagination['page'];$pages=(int)$pagination['pages'];$pageKey=(string)($pagination['page_key']??'page');$sizeKey=(string)($pagination['size_key']??'per_page');$total=(int)$pagination['total'];$sizes=array_values(array_filter([10,25,50,75,100],static fn(int $size):bool=>$size<=$total));$sizes[]=min($total,100);$sizes=array_values(array_unique($sizes));sort($sizes);$url=static function(int $target)use($pageKey):string{$query=$_GET;$query[$pageKey]=$target;return base_url('index.php?'.http_build_query($query));};?>
