...
$temporaryPasswordDisplay=is_string($temporaryPasswordForDisplay??null)&&$temporaryPasswordForDisplay!==''?$temporaryPasswordForDisplay:null;
...
<div id="adminUsersResults">
    <section class="users-table-card">
        ...
        <?php else:?><div class="users-list">
            ...
            <article class="user-record" data-role="<?=e($user['role_code'])?>" data-status="<?=e($user['status'])?>" data-search-text="<?=e(implode(' ',[$user['username']??'',$user['full_name'],(string)($user['email']??'')]))?>" data-search-number="<?=e($user['institutional_code']??'')?>">
                <div class="user-identity">
                    ...
                    <div>
                        ...
                        <small><?= !empty($user['email']) ? $highlight((string)$user['email'], $filters['search'] ?? '') : 'Correo no registrado' ?></small>
...
<div class="user-modal" id="userModal" hidden><div class="user-modal-card" role="dialog" aria-modal="true" aria-labelledby="userModalTitle">
    ...
    <form id="userForm"><input type="hidden" name="_csrf" value="<?=e($adminUserCsrf)?>"><input type="hidden" name="id" value=""><input type="hidden" name="career_id" value="<?= (int)($career['id']??0)?>"><input type="hidden" name="academic_period_id" value="<?= (int)($period['id']??0)?>">
        ...
        <div class="form-grid">
            ...
            <label class="wide"><span>Correo <small>(opcional)</small></span><input type="email" name="email" maxlength="190" autocomplete="off"></label>
        ...
        <div class="form-note" id="temporaryPasswordNote"><i class="fa-solid fa-key"></i><span><?php if($temporaryPasswordDisplay!==null):?>La cuenta se creará con la contraseña temporal <strong><?=e($temporaryPasswordDisplay)?></strong> y deberá cambiarla.<?php else:?><strong>Requiere configuración.</strong> Registra la contraseña temporal institucional en Admin &gt; Configuración &gt; Acceso inicial antes de crear cuentas.<?php endif;?></span></div><div class="users-message" id="userFormMessage" hidden></div>
