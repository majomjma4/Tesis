<?php
$roleLabels=['student'=>'Estudiante','teacher'=>'Docente','administrator'=>'Administrador'];
...
<div class="user-modal" id="userModal" hidden><div class="user-modal-card" role="dialog" aria-modal="true" aria-labelledby="userModalTitle">
    ...
    <form id="userForm"><input type="hidden" name="_csrf" value="<?=e($adminUserCsrf)?>"><input type="hidden" name="id" value=""><input type="hidden" name="career_id" value="<?= (int)($career['id']??0)?>"><input type="hidden" name="academic_period_id" value="<?= (int)($period['id']??0)?>">
        ...
        <div class="form-grid">
            ...
            <div class="role-fields wide" data-for="student teacher"><label><span>Cédula</span><input name="institutional_code" inputmode="numeric" minlength="10" maxlength="10" pattern="[0-9]{10}" placeholder="10 dígitos"></label></div>
            <div class="role-fields student-fields wide" data-for="student">
                ...
                <div class="user-fixed-field"><span>Carrera</span><strong><?=e($career['name']??'Desarrollo de Software')?></strong></div>
                <div class="user-fixed-field"><span>Periodo académico</span><strong><?=e($period['name']??'Sin periodo activo')?></strong></div>
                <label><span>Semestre</span><select name="semester"><option value="">Selecciona</option><?php for($semester=1;$semester<=10;$semester++):?><option value="<?=$semester?>"><?=$semester?>.º semestre</option><?php endfor;?></select></label>
            </div>
